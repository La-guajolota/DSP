...
def plot_filter_response(h, title="Respuesta del Filtro", fs=1.0):
    ...
    H_mag = np.abs(H)
    H_db = 20 * np.log10(H_mag + 1e-10)  # Evitar log(0)
...
def generate_test_signal(fs=1000, duration=1.0, frequencies=None):
    """
...
def design_filter(filter_type, cutoff, order, window='hamming', fs=1.0):
    ...
    """
    order += 1  # Asegurar que el orden sea impar para fase lineal
    ...
    if filter_type.lower() == 'lowpass':
        ...
        # Asegurar que cutoff es una tupla
...
def create_output_folder(section_name):
    """
    Crea una carpeta para almacenar los resultados según la sección solicitada.
    """
    folder_path = os.path.join(os.getcwd(), section_name)
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    return folder_path


# Ejemplos de diseño de filtros

def ejemplo_paso_bajas():
    section_name = "Paso_Bajas"
    output_folder = create_output_folder(section_name)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_bajas.png"))
    plt.show()
    ...
    fig.savefig(os.path.join(output_folder, "paso_bajas_hamming.png"))
    export_filter_coeffs(h_hamming, os.path.join(output_folder, "lowpass_coeffs.csv"))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_bajas_senal.png"))
    plt.show()


def ejemplo_paso_altas():
    section_name = "Paso_Altas"
    output_folder = create_output_folder(section_name)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_altas.png"))
    plt.show()

    # Para la ventana Hamming, exportar coeficientes
    h_hamming = design_filter('highpass', cutoff, order, 'hamming', fs)
    fig = plot_filter_response(h_hamming, "Paso-Altas Hamming", fs)
    fig.savefig(os.path.join(output_folder, "paso_altas_hamming.png"))
    export_filter_coeffs(h_hamming, os.path.join(output_folder, "highpass_coeffs.csv"))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_altas_senal.png"))
    plt.show()


def ejemplo_paso_banda():
    section_name = "Paso_Banda"
    output_folder = create_output_folder(section_name)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_banda.png"))
    plt.show()

    # Para la ventana Hamming, exportar coeficientes
    h_hamming = design_filter('bandpass', cutoff, order, 'hamming', fs)
    fig = plot_filter_response(h_hamming, "Paso-Banda Hamming", fs)
    fig.savefig(os.path.join(output_folder, "paso_banda_hamming.png"))
    export_filter_coeffs(h_hamming, os.path.join(output_folder, "bandpass_coeffs.csv"))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, "ejemplo_paso_banda_senal.png"))
    plt.show()
